...
将 [OrganonCore/AGENTS.md](../OrganonCore/AGENTS.md) 与本文件一并阅读，包括其链接的哲学、审查方法和[迭代规则](../OrganonCore/skills/references/iteration.md)。外层仓库工作适用共享的维护、独立审查、措辞、授权、记录和停止规则。Core 负责 Lean 实现、检查工具及共享的纯解析逻辑；外层仓库提供哲学管理脚本和保持兼容的 Lean 转发入口。两个仓库的产品运行工具均无第三方运行依赖。站点构建与验证可使用锁定的第三方依赖；这些依赖不进入安装包。
...
读取本地记录时，从相应仓库的 `.local/ACTIVE.md` 开始。反馈记录保持活跃。默认递归搜索排除 `.local/archived/`，默认不打开归档包。任务明确需要历史材料时，查阅 `.local/archived/INDEX.md`，只取用相关包或成员，并遵循其中记录的依赖和恢复条件。已归档的决定是历史证据，不是活跃义务；后续更正另立记录并关联原件。
